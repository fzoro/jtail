...
public class FileConf {
	...
	for (Object key : keys) {
	    strKey = key.toString();
	    currentFileOnMap = mapFiles.get(strKey.replaceAll(
		    Constants.Def.MAX_BUFFER_SUFIX, ""));
		...
		currentMaxBuffer = NumberUtil.toInteger(Prop.get()
			.getProperties().getProperty(strKey));
		currentFileOnMap.maxBuffer(currentMaxBuffer);
		strKey = strKey.substring(0, strKey.length()
			- Constants.Def.MAX_BUFFER_SUFIX.length());
	    } else {
