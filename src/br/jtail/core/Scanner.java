...
public final class Scanner {
     ...
     */
    public static DataFile readToDataFile(final String fileName,
	    final String filePath, int skip) throws JTailException {
     ...
     * @throws IOException
     */
    private static void maxBuffer(String fileName, DataInputStream dis,
	    int fileLength) throws JTailException, IOException {

	String fileMaxBuffer = fileName + Constants.Def.MAX_BUFFER_SUFIX;

	int maxKBytes = Constants.Def.NEGATIVE_INDEX;
	try {
	    String maxBuffer = Prop.get().getProperties()
		    .getProperty(fileMaxBuffer);
	    if (maxBuffer != null) {
	...
	public String toString() {
	    // return
	    // String.format("{ \"fileName\" : \"%s\", \"off\":\"%d\", \"value\":\"%s\"}",
	    // this.fileName, this.bytes, this.value);
	    return new JSONStringer().object().key("fileName")
		    .value(this.fileName).key("off").value(this.bytes)
		    .key("value").value(this.value).endObject().toString();
